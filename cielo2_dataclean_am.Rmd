...
# importing our data
d_all <- read_csv("/Users/ashlynnmissimo/Desktop/cielo2_vigdata.csv")
...
d_all<-d_all[(d_all$sort_god==1),]


# now we want to view our dataframe in the console to make sure we removed the right particiants. great!
...
d_all<-subset(d_all, pid != 152 & pid != 211 & pid != 234 & pid != 245 & pid != 248 & pid != 261 & pid != 293)
# up to this point, the data excludes kids who were distracted or coached - but not those who said god was pretend.
```
...
d_sort <- dplyr::select(d_all, gender, sort_germ, sort_tooth, sort_vitamin, sort_mermaid, sort_elf, sort_fairy, sort_soul, sort_god, sort_santa, contains("pid"))
...
# here, we're rewriting over the data frame d_exist that we just created above and essentially getting rid of the 1 participant that selected gender as "other" in the previous data frame 
...
```{r exporting data}
# below is the file for cleaned data that excludes participants who were distracted/coached, but includes god is pretend
# commented the data file out so it doesn't keep saving it every time I run it 
# write.csv(d_all,"\\cielo2_cleaneddata.csv", row.names = FALSE)
```

``` {r exporting real data}
d_all_real<-subset(d_all, sort_god != 2)
d_all_real<-subset(d_all_real, pid != 202)
# above excluded participant 202 because in p. log, Matt noted  "god is pretend"... but chose "real" in Qualtrics
# write.csv(d_all_real,"\\cielo2_cleaneddata_real.csv", row.names = FALSE)
```
...
sortgerm_plot <- ggplot(d_sort, aes(as.character(sort_germ))) +
  geom_bar(fill='light blue') +
  labs(x='Real or Pretend', y="Participants", title ="Are germs real?") +
  theme_light()+
  scale_x_discrete(labels=c('Real', 'N/A'))
sortgerm_plot

# ashley will practice making more plots in another markdown script
